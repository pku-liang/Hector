  ...
  for (auto &op : llvm::reverse(block)) {
    if (auto ifOp = llvm::dyn_cast<tor::IfOp>(op)) {
      ...
      if (loadOp.getOperation()->hasAttr("dependence")) {
        auto deps = loadOp.getOperation()
                        ->getAttr("dependence")
                        .cast<tor::DependenceAttr>();
      ...
      if (storeOp.getOperation()->hasAttr("dependence")) {
        auto deps = storeOp.getOperation()
                        ->getAttr("dependence")
                        .cast<tor::DependenceAttr>();
        signatures = deps.getSignatures();
        distances = deps.getDistances();
      ...
      OpAbstract *newOpA = createMemOp(
          ...
          OpAbstract::OpType::STORE_OP, signatures, distances);
      OperationMap[&op] = newOpA;
      lastHead->addOperation(newOpA);
      ...
      BasicBlock::addControlDependency(
          {bodyBB.second, bodyBB.first, ControlEdge::LOOPBACK});
      BasicBlock::addControlDependency(
          {bodyBB.first, lastHead, ControlEdge::COND});

      auto yieldOp = forOp.getBody()->getTerminator();

      Value induction = forOp.getInductionVar();
      OpAbstract *newOpA =
          createOp(&op, curLoop, bodyBB.first, std::vector<Value>{induction},
      ...
      for (auto iter : llvm::enumerate(forOp.getRegionIterArgs())) {
        Value x = iter.value();
        OpAbstract *newOpA =
            createOp(&op, curLoop, bodyBB.first, std::vector<Value>{x},
      ...
      lastHead = BasicBlocks.back().get();
      BasicBlock::addControlDependency(
          ControlEdge(lastHead, bodyBB.first, ControlEdge::FORWARD));
...
void ScheduleBase::buildDFG() {
  ...
  for (auto &&op1 : Operations) {
    ...
    for (auto &&op2 : Operations) {
      ...
      // memop1 can't reach memop2 (e.g. mutually exclusive branch of an if
      // statement)
      if (Distance == -1)
        continue;
